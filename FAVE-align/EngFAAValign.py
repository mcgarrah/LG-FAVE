...
def align(wavfile, trs_input, outfile, FADIR='', SOXPATH='', HTKTOOLSPATH=''):
    ...
    try:
        ...
        if HTKTOOLSPATH:  ## if absolute path to HTK Toolkit is given
            os.system(os.path.join(HTKTOOLSPATH, 'HCopy') + ' -T 1 -C ./model/' + str(
                SR) + '/config -S ./tmp' + identifier + '/codetr' + identifier + '.scp >> ./tmp' + identifier + '/blubbeldiblubb.txt')
            os.system(os.path.join(HTKTOOLSPATH,
                                   'HVite') + ' -T 1 -a -m -I ./tmp' + identifier + '/tmp' + identifier + '.mlf -H ./model/' + str(
                SR) + '/macros -H ./model/' + str(
                SR) + '/hmmdefs  -S ./tmp' + identifier + '/test' + identifier + '.scp -i ./tmp' + identifier + '/aligned' + identifier + '.mlf -p 0.0 -s 5.0 ' + options.dict + ' ./model/monophones > ./tmp' + identifier + '/aligned' + identifier + '.results')
            ...
            os.system('HCopy -T 1 -C ./model/' + str(
                SR) + '/config -S ./tmp' + identifier + '/codetr' + identifier + '.scp >> ./tmp' + identifier + '/blubbeldiblubb.txt')
            os.system('HVite -T 1 -a -m -I ./tmp' + identifier + '/tmp' + identifier + '.mlf -H ./model/' + str(
                SR) + '/macros -H ./model/' + str(
                SR) + '/hmmdefs  -S ./tmp' + identifier + '/test' + identifier + '.scp -i ./tmp' + identifier + '/aligned' + identifier + '.mlf -p 0.0 -s 5.0 ' + options.dict + ' ./model/monophones > ./tmp' + identifier + '/aligned' + identifier + '.results')
...
def aligned_to_TextGrid(infile, outfile, SR):
    ...
    while (lines[j] <> '.\n'):
        ...
        if (SR == 11025):  ## adjust rounding error for 11,025 Hz sampling rate
            ...
            st = round((round(float(lines[j].split()[0]) / 10.0, 0) / 1000000.0) * (11000.0 / 11025.0) + 0.0125,
                       3)  ## start time
            en = round((round(float(lines[j].split()[1]) / 10.0, 0) / 1000000.0) * (11000.0 / 11025.0) + 0.0125,
                       3)  ## end time
        else:
        ...
        if is_sound(args[0]):
            ...
            if len(args) == 1:  ## only sound file given
                ...
                if is_text(args[1]):  ## sound file and transcription file given
                    ...
                    tgfile = args[1]
                    trsfile = check_file(
                        replace_extension(wavfile, ".txt"))  ## transcription file name must match sound file
        ...
        ## case B:  unknown words check, no sound file
            ...
            if (__name__ == "__main__" and is_text(args[0])) or (__name__ != "__main__" and is_text(args[1])):
                if (__name__ == "__main__" and is_text(args[0])):
                    ...
                    trsfile = check_file(args[1])
                tgfile = replace_extension(trsfile,
                                           ".TextGrid")  ## need to have a name for the TextGrid for the name of the outputlog (renamed from original name of the TextGrid later)
            else:
...
def check_dictionary_entries(lines, wavfile):
    ...
    for line in lines:
        ...
        (words, flag_uncertain, last_beg_uncertain, last_end_uncertain) = preprocess_transcription(line.strip().upper(),
                                                                                                   flag_uncertain,
                                                                                                   last_beg_uncertain,
                                                                                                   last_end_uncertain)
    ...
    if not options.check:
        global temp_dict
        temp_dict = os.path.join(os.path.dirname(wavfile),
                                 '_'.join(os.path.basename(wavfile).split('_')[:2]) + "_" + "dict")
...
def cut_chunk(wavfile, outfile, start, dur, SOXPATH):
    ...
    if SOXPATH:
        command_cut_sound = " ".join(
            [SOXPATH, '\"' + wavfile + '\"', '\"' + outfile + '\"', "trim", str(start), str(dur)])
        ...
        command_cut_sound = " ".join(
            ["sox", '\"' + wavfile + '\"', '\"' + outfile + '\"', "trim", str(start), str(dur)])
    try:
...
def define_options_and_arguments():
    ...
    parser.add_option('-i', '--import', help=import_help, metavar='FILENAME',
                      dest='importfile')  ## required argument FILENAME
...
def delete_empty_lines(delete_lines, all_input):
    ...
    for dline in delete_lines:
        ...
        while p < len(all_input):
            ...
            if (len(o) >= 4 and (o[0] == d[0]) and (o[1] == d[1]) and (o[2] == d[2]) and (o[3] == d[3])) or not o[
                0].strip():
                all_input.pop(p)
                ## get out of the loop
                break
...
def get_duration(soundfile, FADIR=''):
    ...
    try:
        ...
        duration = round(
            float(subprocess.Popen(dur_command, shell=True, stdout=subprocess.PIPE).communicate()[0].strip()), 3)
    ...
    for word in words:
        if word == "((":  ## beginning of uncertain transcription span
            if not flag_uncertain:
                ...
                error = "ERROR!  Beginning of uncertain transcription span detected twice in a row:  %s.  Please close the the opening double parenthesis in line %s." % (
                original_line, last_beg_uncertain)
                errorhandler(error)
...
def reinsert_uncertain(tg, text):
    ...
    for (n, entry) in enumerate(tgwords):
        ...
        ## original transcription contains uncertain transcription:
            ...
            if tgword == uncertain.sub(r'\1', text[n]).lstrip('*') and tg[1][tgposition].mark() == uncertain.sub(r'\1',
                                                                                                                 text[
                                                                                                                     n]).lstrip(
                    '*'):
                tg[1][tgposition].change_text(text[n])
...
def FAAValign(opts, args, FADIR='', SOXPATH=''):
    ...
    if not (len(trans_lines) == len(all_input)):
        error = "ERROR!  Length of input data lines (%s) does not match length of transcription lines (%s).  Please delete empty transcription intervals." % (
        len(all_input), len(trans_lines))
        errorhandler(error)
    ...
    for (text, line) in zip(trans_lines, all_input):
        ...
        beg = round(float(entries[2]), 3)
        end = min(round(float(entries[3]), 3),
                  duration)  ## some weird input files have the last interval exceed the duration of the sound file
        dur = round(end - beg, 3)
        if options.verbose:
            try:
                ...
                print "Processing %s -- chunk %i:  %s" % (
                speaker, count_chunks, " ".join(text).encode('ascii', 'replace'))
        ...
        chunkname_sound = "_".join([os.path.splitext(os.path.basename(wavfile))[0], speaker.replace(" ", "_"), "chunk",
                                    str(count_chunks)]) + ".wav"
        ...
        ## align chunk
        try:
            align(os.path.join(tempdir, chunkname_sound), [text], os.path.join(tempdir, chunkname_textgrid), FADIR,
                  SOXPATH, HTKTOOLSPATH)
        except Exception, e:
            try:
                print "\tERROR!  Alignment failed for chunk %i (speaker %s, text %s)." % (
                count_chunks, speaker, " ".join(text))
            except (UnicodeDecodeError, UnicodeEncodeError):
                print "\tERROR!  Alignment failed for chunk %i (speaker %s, text %s)." % (
                count_chunks, speaker, " ".join(text).encode('ascii', 'replace'))
