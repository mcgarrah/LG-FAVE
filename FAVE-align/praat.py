...
class Formant:
  ...
  def read(self, file):
    ...
    text = io.open(file, 'rU', encoding='utf8')
...
class TextGrid:
    ...
    def read(self, filename):
        ...
        text = io.open(filename, 'rU', encoding='utf8')
    ...
    def write(self, text):  
        ...
        text = io.open(text, 'w', encoding='utf8')
        text.write(u'File type = "ooTextFile"\n')
        text.write(u'Object class = "TextGrid"\n\n')
        text.write(u'xmin = %f\n' % self.__xmin)
        text.write(u'xmax = %f\n' % self.__xmax)
        text.write(u'tiers? <exists>\n')
        text.write(u'size = %d\n' % self.__n)
        text.write(u'item []:\n')
        for (tier, n) in zip(self.__tiers, range(1, self.__n + 1)):
            text.write(u'\titem [%d]:\n' % n)
            if tier.__class__ == IntervalTier: 
                text.write(u'\t\tclass = "IntervalTier"\n')
                text.write(u'\t\tname = "%s"\n' % tier.name())
                text.write(u'\t\txmin = %f\n' % tier.xmin())
                text.write(u'\t\txmax = %f\n' % tier.xmax())
                text.write(u'\t\tintervals: size = %d\n' % len(tier))
                for (interval, o) in zip(tier, range(1, len(tier) + 1)): 
                    text.write(u'\t\t\tintervals [%d]:\n' % o)
                    text.write(u'\t\t\t\txmin = %f\n' % interval.xmin())
                    text.write(u'\t\t\t\txmax = %f\n' % interval.xmax())
                    text.write(u'\t\t\t\ttext = "%s"\n' % interval.mark())
            else: # PointTier
                text.write(u'\t\tclass = "TextTier"\n')
                text.write(u'\t\tname = "%s"\n' % tier.name())
                text.write(u'\t\txmin = %f\n' % tier.xmin())
                text.write(u'\t\txmax = %f\n' % tier.xmax())
                text.write(u'\t\tpoints: size = %d\n' % len(tier))
                for (point, o) in zip(tier, range(1, len(tier) + 1)):
                    text.write(u'\t\t\tpoints [%d]:\n' % o)
                    text.write(u'\t\t\t\ttime = %f\n' % point.time())
                    text.write(u'\t\t\t\tmark = "%s"\n' % point.mark())
        text.close()
...
class IntervalTier:
    ...
    def read(self, file):
        text = io.open(file, 'r', encoding='utf8')
        text.readline() # header junk 
        text.readline()
        text.readline()
    ...
    def write(self, file):
        text = io.open(file, 'w', encoding='utf8')
        text.write(u'File type = "ooTextFile"\n')
        text.write(u'Object class = "IntervalTier"\n\n')
        text.write(u'xmin = %f\n' % self.__xmin)
        text.write(u'xmax = %f\n' % self.__xmax)
        text.write(u'intervals: size = %d\n' % self.__n)
...
class PointTier:
    ...
    def write(self, file):
        text = io.open(file, 'w', encoding='utf8')
        text.write(u'File type = "ooTextFile"\n')
        text.write(u'Object class = "TextTier"\n\n')
        text.write(u'xmin = %f\n' % self.__xmin)
        text.write(u'xmax = %f\n' % self.__xmax)
        text.write(u'points: size = %d\n' % self.__n)
