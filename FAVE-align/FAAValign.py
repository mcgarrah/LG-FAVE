...
-d [filename], --dict=[filename] ("dictionary"):
    ...
    Specifies the name of the file containing the pronunciation dictionary.  Default file is "./model/en_US/dict".
...
# TODO: Generalize the CONSONANTS and VOWELS to a the --lang configuration setting
#CONSONANTS = ['B', 'CH', 'D', 'DH','F', 'G', 'HH', 'JH', 'K', 'L', 'M', 'N', 'NG', 'P', 'R', 'S', 'SH', 'T', 'TH', 'V', 'W', 'Y', 'Z', 'ZH']
#CONSONANTS = ['B', 'BB', 'CH', 'D', 'DD', 'DH', 'F', 'G', 'GG', 'HH', 'J', 'JH', 'K', 'KK', 'L', 'M', 'N', 'NG', 'P', 'PP', 'R', 'RD', 'RL', 'RN', 'RS', 'RT', 'RX', 'S', 'SJ', 'T', 'TT', 'TH', 'TJ', 'V', 'W', 'Z', 'ZH']
...
# TODO: Modify the later usage of global TEMPDIR below to use "import tempfile"
#TEMPDIR = "temp_FA"
TEMPDIR = ""
# TODO: Make language-specific added_dict_entries but change the name to SE_potential_dict_entries, DA_potential_dict_entries, etc.
DICT_ADDITIONS = "added_dict_entries.txt"               ## file for collecting uploaded additions to the dictionary
# TODO: PRAATPATH parameter with a default like below
...
# TODO: Add PRAATPATH as a parameter to align() with default of '' like HTKTOOLPATH
def align(wavfile, trs_input, outfile, FADIR='', SOXPATH='', PRAATPATH='', HTKTOOLSPATH=''):
    ...
    # TODO: Use "import tempfile" and https://stackoverflow.com/questions/3223604/how-to-create-a-temporary-directory-and-get-the-path-file-name-in-python
    ...
    try:
        ...
        #DEFAULT_MODEL_PATH = 'model/en_US'  # DEBUG LINE
        if HTKTOOLSPATH:  ## if absolute path to HTK Toolkit is given
            os.system(os.path.join(HTKTOOLSPATH, 'HCopy') + ' -T 1 -C ' + os.path.join('.',DEFAULT_MODEL_PATH, str(SR), 'config') + ' -S ./tmp' + identifier + '/codetr' + identifier + '.scp >> ./tmp' + identifier + '/blubbeldiblubb.txt')
            os.system(os.path.join(HTKTOOLSPATH, 'HVite') + ' -T 1 -a -m -I ./tmp' + identifier + '/tmp' + identifier +'.mlf -H ' + os.path.join('.',DEFAULT_MODEL_PATH, str(SR), 'macros') + ' -H ' + os.path.join('.',DEFAULT_MODEL_PATH, str(SR), 'hmmdefs') + ' -S ./tmp' + identifier + '/test' + identifier+ '.scp -i ./tmp' + identifier + '/aligned' + identifier + '.mlf -p 0.0 -s 5.0 ' + options.dict + ' ' + os.path.join('.',DEFAULT_MODEL_PATH, 'monophones') + ' > ./tmp' + identifier + '/aligned' + identifier + '.results')
            ...
            os.system('HCopy -T 1 -C ' + os.path.join('.',DEFAULT_MODEL_PATH, str(SR), 'config') + ' -S ./tmp' + identifier + '/codetr' + identifier + '.scp >> ./tmp' + identifier + '/blubbeldiblubb.txt')
            os.system('HVite -T 1 -a -m -I ./tmp' + identifier + '/tmp' + identifier +'.mlf -H ' + os.path.join('.',DEFAULT_MODEL_PATH, str(SR), 'macros') + ' -H ' + os.path.join('.',DEFAULT_MODEL_PATH, str(SR), 'hmmdefs') + ' -S ./tmp' + identifier + '/test' + identifier+ '.scp -i ./tmp' + identifier + '/aligned' + identifier + '.mlf -p 0.0 -s 5.0 ' + options.dict + ' ' + os.path.join('.',DEFAULT_MODEL_PATH, 'monophones') + ' > ./tmp' + identifier + '/aligned' + identifier + '.results')
...
def check_phone(p, w, i):
    ...
    # TODO: Get the maximum length of a CONSONANT phone value
    # TODO: Get the number of CONSONANT phone values
    CONS_MAX_SIZE = len(max(CONSONANTS, key=len))           # sv_SE 2
                                                            # en_US 2
    VOWL_MAX_SIZE = len(max(VOWELS, key=len))               # sv_SE 3
                                                            # en_US 2
    # TODO: the hardcoded 2 below should be a MIN vowl length
    VOWEL_STRESS_LIST = map(str, range(VOWEL_STRESS_COUNT)) # sv_SE 5 ['0', '1', '2', '3', '4']
                                                            # en_US 3 ['0', '1', '2']

    # TODO: Fix length check for phone to not be hard coded to '3' but based on check of phone structure
    # TODO: Fix length check for VOWELS counts to be dynamic based on structure passed
    # TODO: Fix length check for CONSONANTS counts to be dynamic based on structure passed
    # TODO: Fix that we have an assumption of no more than 0-9 numerics (one char numbers) on phones
    # if not ( (len(p) == 3 and p[-1] in ['0', '1', '2'] and p[:-1] in VOWELS)
    #         or (len(p) <= 2 and p in CONSONANTS) ):
    if not ( ( ((len(p) <= VOWL_MAX_SIZE + 1) or (len(p) >= 2 )) and p[-1] in VOWEL_STRESS_LIST and p[:-1] in VOWELS)
            or (len(p) <= CONS_MAX_SIZE and p in CONSONANTS) ):
        ## check whether transcriber didn't simply forget the stress coding for vowels:
        if __name__ == "__main__":
            # TODO: This error check is based on length of phone requires a lookup now because phones can be variable sized.
            # XX1  true
            # XX   false
            # YYY1 true
            # YYY  false
            # Check for digit on end of 'p'
            # Verify max_len + 1
            # if != (p in VOWELS)[01234], then scary message
            #if len(p) == VOWL_MAX_SIZE and p[:-1] in VOWELS:
            if not (p[:-1] in VOWELS):
                print "You either forgot to enter the stress digit for vowel %s (at position %i) in word %s!\nor you have an incorrect monophone.\n" % (p, i+1, w)
...
def check_transcription(w):
    ...
    # TODO: Fix the phone len(w) check to be dynamic not a hard coded '3' by optional '5'
    #if len(w) > 3 and len(phones) < 2:

    CONS_MAX_SIZE = len(max(CONSONANTS, key=len))           # sv_SE 2
    VOWL_MAX_SIZE = len(max(VOWELS, key=len))               # sv_SE 3
    PHONE_MAX_SIZE = max(CONS_MAX_SIZE, VOWL_MAX_SIZE)

    if len(w) > PHONE_MAX_SIZE + 1 and len(phones) < 2:
...
# TODO: Create a function to generalize the language specific character replacement
# TODO: Section 4.6.3 Updating the pronunciation dictionary : Adding rules for very-happy.
def replace_special_characters(all_input):
    # TODO: evaluate the eval() and exec() as options for building transcription rewrite rules
    """

    Private and Public namespaces for eval and exec
    https://stackoverflow.com/questions/701802/how-do-i-execute-a-string-containing-python-code-in-python/14926430#14926430

    Executing Code and saving results
        kw = {}
        exec("ret = 4") in kw
        kw['ret']
...
def check_word(word, next_word='', unknown={}, line=''):
    ...
    if truncated.search(word) and word in cmudict:
        ...
        # TODO: Extract English only ING to IN rule into Language Generalized Code.
        if ing.search(word):
...
def cut_chunk(wavfile, outfile, start, dur, SOXPATH):
    ...
    # TODO: Add a check for the SoX binary being functional before this code executes and return an error message.
    if SOXPATH:
...
def define_options_and_arguments():
    ...
    # TODO: Add --lang option help prompt information
    ...
    ipa_help = """Convert phones to IPA representations as supported by language."""
    dict_help = """Specifies the name of the file containing the pronunciation dictionary.  Default file is "/model/dict"."""
    lang_help = """Specifies the locale id of the language definition.  Default locale id is "en_US"."""
    ...
    praatpath_help = """Specifies the path to the Praat install where the Praat executable files are located.  If not specified, the user's path will be searched for the location of the executable."""
    #soxpath_help = """Copy above line text for SOXPATH"""

    parser = optparse.OptionParser(usage=new_use, description=new_desc, epilog=new_ep, version=vers)
    # TODO: Add -l | --lang option parameter with default of 'en_US' metavar of LANGUAGE
    # TODO: Fix the --lang with a similar method to the model/dict for --dict for loading language settings using FILENAME
    ...
    parser.add_option('-a', '--ipa', action='store_true', default=False, help=ipa_help)
    parser.add_option('-d', '--dict', default='model/en_US/dict', help=dict_help, metavar='FILENAME')
    parser.add_option('-l', '--lang', default='en_US', type='choice', choices=['en_US','sv_SE', 'da_DK'], help=lang_help, metavar='LANG_ID')
    ...
    parser.add_option('-p', '--praatpath', default='', help=praatpath_help, metavar='PRAATPATH')
    #parser.add_option('-s', '--soxpath', default='', help=soxpath_help, metavar='SOXPATH')
    ...
    # - "bool" options.ipa (default:  False)
    # - string options.dict (default:  "model/dict")
    # - "bool" options.noprompt (default:  False)
    # - string options.htktoolspath (default: '')
    # - string options.praatspath (default: '')
    # - string options.lang (default: 'lang/en_US')

    # TODO: Verify that -t and --htktoolspath also produces above types then document above.
    # TODO: Add the --lang and -l options above to docs
    # TODO: Add a --praatpath like the --htktoolpath instead of hard coding it. Currently not working.
...
def get_duration(soundfile, PRAATPATH='', FADIR=''):
...
def merge_textgrids(main_textgrid, new_textgrid, speaker, chunkname_textgrid):
    ...
    # TODO: Add a converter for ARPABET to IPA format for a second TextGrid file.
    # TODO: This could also be called from the higher level FAAValign()
...
def preprocess_transcription(line, flag_uncertain, last_beg_uncertain, last_end_uncertain):
    ...
    line = line.replace(' - ', '')
    ## put space at beginning and end of string
    ...
    # Added for language independent execution
    line = transcript_preprocessor(line)
...
# This function reads a language configuration file
def read_language_file(langfile):
    import ConfigParser
    lang_dict = {}
    config = ConfigParser.ConfigParser()
    config.read(langfile)
...
def read_transcription_file(trsfile):
    ...
    # TODO: Fix the codec.opens() with io.open() in Python 2.7.x and 3.x
    ...
    try:  ## try UTF-16 encoding first
        ...
        lines = t.readlines()
        # TODO: add replace_special_characters()
    except UnicodeError:
        try:  ## then UTF-8...
    ...
    # Global Replace of transcript representations
    lines = transcript_representation_rules(lines)
...
def reinsert_uncertain(tg, text):
    ...
    # list text = list of words that should correspond to entries on word tier of tg
    # (original transcription WITH parentheses, asterisks etc.)
    # OUTPUT:
...
def tidyup(tg, beg, end, tgfile):
    """extends the duration of a TextGrid and all its tiers from begin to end;
...
def write_log(filename, wavfile, duration):
    ...
    try:
        ...
        # TODO: Fix the version retrieve to use something that can be tracked locally. GIT is not always reliable.
    ...
    try:
        # TODO: Fix GIT issue below using same method as above
...
def FAAValign(opts, args, FADIR='', SOXPATH=''):
    ...
    # read in the language configuration file
    LANG_ID = options.lang
    if options.lang <> 'en_US':
        options.dict = 'model/' + options.lang + '/dict'
    if options.verbose:
        print "Dict file %s." % options.dict
    lang_path = os.path.join(FADIR, 'model', LANG_ID)
    lang_file = os.path.join(LANG_ID + '.py')

    # Code for language features load
    import importlib
    import sys
    sys.path.append(lang_path)
    lang_import = importlib.import_module(LANG_ID)
    locals().update(lang_import.__dict__)
    globals().update(lang_import.__dict__)
    if options.verbose:
        print "Imported language features file %s." % os.path.join(lang_path, lang_file)

    ## read CMU dictionary
    ## (default location is "./model/en_US/dict", unless specified otherwise via the "--dict" option)
    global cmudict
    cmudict = read_dict(os.path.join(FADIR, options.dict))
    if options.verbose:
        print "Imported dictionary file %s." % os.path.join(FADIR, options.dict)
    ...
    # TODO: Add PraatPath from command line options
    PRAATPATH = options.praatpath
    # TODO: Add SoX path from command line options
    #SOXPATH = options.soxpath
    ...
    duration = get_duration(wavfile, PRAATPATH, FADIR)
    if options.verbose:  
    ...
    for (text, line) in zip(trans_lines, all_input):
        ...
        # TODO: Add sox check for existence of binary before execute of cut_chunk() below.
        ...
        ## align chunk
        try:
            # TODO: Add PRAATPATH as a parameter to align() with default of ''
            align(os.path.join(tempdir, chunkname_sound), [text], os.path.join(tempdir, chunkname_textgrid), FADIR, SOXPATH, PRAATPATH, HTKTOOLSPATH)
        except Exception, e:
            try:
    ...
    # TextGrid representation reversion - transcript
    # DONE: This is were the TextGrid data structure is modified to change back representations
    main_textgrid = textgrid_reinsert_representation(main_textgrid, None)
    if options.verbose:
        print "Reinsert original %s language representations into TextGrid transcript." % options.lang

    # TextGrid representation reversion - phonetics (2IPA)
    # TODO: This is were the TextGrid data structure is modified to change ARPABET representations to IPA
    # TODO: Create a parameter option to enable or disable the IPA conversion. Currently hardwired enabled.
    #if options.convertIPA:
    if options.ipa:
        main_textgrid = textgrid_insert_ipa_representation(main_textgrid, None)
        if options.verbose:
            print "Insert IPA %s language representations into TextGrid phonetics." % options.lang
